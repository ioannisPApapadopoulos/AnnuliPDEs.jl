...
# Exact solution
ua(r,θ) = gbump(r,θ) * jump(r,θ) * (r^2 - α^2)
...
###
# (1 element) Zernike annular discretisation
###
Z = ZernikeAnnulus(α, 1, 1)
wZ = Weighted(Z)
...
###
Z = [ZernikeAnnulus(ρ, 0, 0), ZernikeAnnulus(α/ρ, 0, 0)]
Zd = [ZernikeAnnulus(ρ, 2, 2), ZernikeAnnulus(α/ρ, 2, 2)]
...
###
# (2-element) Spectral element Chebyshev-Fourier + ZernikeAnnulus discretisation
###
T,C,F = chebyshevt(α..ρ),ultraspherical(2, α..ρ),Fourier()
r = axes(T,1)
D = Derivative(r)
...
for n in 11:10:111
    ...
    # We utilise a tau-method to enforce the boundary conditions
    # and continuity.
    u, X = zernikeannulus_chebyshev_fourier_helmholtz_modal_solve([Z, (T,F)], rhs_xy, f, n, [Δ, (Lₜ, M, R, D)], [L, []], λs)

    print("Computed coefficients for n=$n \n")
    collect_errors((Z,T,F,α,ρ), (u,X), ua, errors_TF_Z)
end

###
# Convergence plot
###
ns = [sum(1:b) for b in 11:10:111]
Plots.plot(ns, errors_Z,
    label=L"\mathrm{Zernike \,\, annular \,\, (1 \,\, element)}",
    linewidth=2,
    markershape=:diamond,
    markersize=5,
)
